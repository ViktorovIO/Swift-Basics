...
delete(&diskStorage, "CD-RW")

add(&diskStorage, "CD-RW")
add(&diskStorage, "DVD-RW")
add(&diskStorage, "CD-R")
add(&diskStorage, "CD-R")

print(diskStorage)

delete(&diskStorage, "CD-R")
delete(&diskStorage, "CD-DL")

print(diskStorage)



func add(_ diskStorage: inout [String], _ disk: String) -> Void {
    diskStorage.append(disk)
}

func delete(_ diskStorage: inout [String], _ disk: String) -> Void {
    let temp = disk
    var flag : Bool = false
    ...
    for (key, value) in diskStorage.enumerated() {
        if temp == diskStorage[key] {
            flag = true
            diskStorage.remove(at: key)
    ...
    if flag == false {
        print("Значения \(temp) не существует в массиве")
    }
}
...
let currencyDictionary = pointsToCurrency(userPoints)
print(currencyDictionary)

let currencyDictionaryWithRatio = pointsToCurrency(userPoints, 11)
print(currencyDictionaryWithRatio)

func pointsToCurrency(_ userPoints: [Int : Double], _ ratio: Int = 10) -> [Int : Int] {
    var result = [Int:Int]()
    userPoints.forEach{ (key: Int, value: Double) in
        let money = Int(value) / ratio
        result[key] = money
    }
